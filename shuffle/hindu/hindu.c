...
void shuffle(int *arr, size_t n) {
  ...
  // Hot loop: while both halves have elements
  while (left_idx < mid && right_idx < n) {
    if (bits_left == 0) {
      random_bits = rand();
      bits_left = 31; // Use 31 bits to avoid sign bit issues
    }

    // Use the LSB and shift
    if (random_bits & 1) {
  ...
  // Handle remaining elements from left side
  if (left_idx < mid) {
    memcpy(&temp[result_idx], &arr[left_idx], (mid - left_idx) * sizeof(int));
    result_idx += (mid - left_idx);
  }

  // Handle remaining elements from right side
  if (right_idx < n) {
